...
class ZaloAPI(object):
    ...
    def add_register_handler(func):
        ...
        async def wrapper(self, ctx):
            ...
            if ctx.author_id in self.conversation_handlers:
                ...
                return await handler_info["handler"](
                    ctx, *handler_info["args"], **handler_info["kwargs"]
                )
        ...
        def decorator(func):
            if commands is not None:
                ...
                if isinstance(commands, str):
                    ...
                    self.register_commands.update(
                        {self.prefix + command: func for command in commands}
                    )
    ...
    def register_next_step_handler(self, func, ctx, *args, **kwargs):
        if not ctx.author_id:
            ...
            self.conversation_handlers[ctx.author_id] = {
                "handler": func,
                "args": args,
                "kwargs": kwargs,
            }
    ...
    async def login(self, phone, password, imei, user_agent=None):
        ...
        await self._state.login(phone, password, imei, user_agent=user_agent)
        try:
            self._imei = self._state.user_imei
            self.uid = (await self.fetch_account_info()).profile.get(
    ...
    async def _getGroupBoardList(
        self, board_type, page, count, last_id, last_type, groupId
    ):
        params = {
            "params": self._encode(
                {
    ...
    async def get_group_board_list(
        self, groupId, page=1, count=20, last_id=0, last_type=0
    ):
        ...
        Raises:
                ZaloAPIException: If request failed
        """
        response = await self._getGroupBoardList(
            0, page, count, last_id, last_type, groupId
        )
    ...
    async def get_group_pinmsg(self, groupId, page=1, count=20, last_id=0, last_type=0):
        ...
        """
        response = await self._getGroupBoardList(
            2, page, count, last_id, last_type, groupId
        )
    ...
    async def get_group_note(self, groupId, page=1, count=20, last_id=0, last_type=0):
        ...
        """
        response = await self._getGroupBoardList(
            1, page, count, last_id, last_type, groupId
        )
    ...
    async def get_group_poll(self, groupId, page=1, count=20, last_id=0, last_type=0):
        ...
        """
        response = await self._getGroupBoardList(
            3, page, count, last_id, last_type, groupId
        )
    ...
    async def send(
        ...
        if message.mention:
            ...
            return await self.send_message(
                message, thread_id, thread_type, mark_message, ttl
            )

    async def send_message(
        self, message, thread_id, thread_type, mark_message=None, ttl=0
    ):
        ...
        Raises:
                ZaloAPIException: If request failed
        """
        params = {"zpw_ver": 647, "zpw_type": 30, "nretry": 0}
                        ...
                        fileSize = (
                            ...
                            if fileSize
                                ...
                                response.headers.get(
                                    "Content-Length", len(await response.read())
                                )
                            )
                        )
                    else:
    ...
    async def send_remote_voice(
        self, voiceUrl, thread_id, thread_type, fileSize=None, ttl=0
    ):
        """Send voice by url.
    ...
    async def send_multi_local_image(
        self,
        imagePathList,
        thread_id,
        thread_type,
        ...
        for i, imagePath in enumerate(imagePathList):
            ...
            data = await self.send_local_image(
                imagePath,
                thread_id,
                thread_type,
                width,
    ...
    async def mark_as_delivered(self, ctx):
        ...
        if results:
            context = ContextObject.fromDict(
                {"msgId": msgId, "thread_id": thread_id, "thread_type": thread_type}
            )
                    ...
                    for message in messages + groupmsg:
                        if (
                            ...
                            if message in messages:
                                ...
                                context = {
                                    "message_id": msgObj.msgId,
                                    "author_id": str(int(msgObj.uidFrom) or self.uid),
                                    "message": msgObj.content,
                                    "message_object": msgObj,
                                    "thread_id": str(
                                        int(msgObj.uidFrom) or msgObj.idTo
                                    ),
                                ...
                                context = {
                                    ...
                                    "message": msgObj.content,
                                    "message_object": msgObj,
                                    "thread_id": str(int(msgObj.idTo) or msgObj.idTo),
                    ...
                    while not self._condition.is_set():
                        try:
                            ...
                            if not hasattr(self, "ws_key"):
                                logger.error(
                                    "Unable to decrypt data because key not found"
                                )
                                continue
                            ...
                            if n == 1 and cmd == 3000 and s == 0:
                                ...
                                for message in userMsgs:
                                    ...
                                    context = {
                                        "message_id": msgObj.msgId,
                                        "author_id": str(
                                            int(msgObj.uidFrom) or self.uid
                                        ),
                                        "message": msgObj.content,
                                        "message_object": msgObj,
                                        "thread_id": str(
                                    ...
                                    [
                                        (
                                            loop.create_task(self.onMessage(context))
                                            if thread
                                            else await self.onMessage(context)
                                        )
                                    ]
                                ...
                                for message in groupMsgs:
                                    ...
                                    try:
                                        messages = (
                                            await self.get_recent_group(message["idTo"])
                                        )["groupMsgs"]
                                        message = next(
                                            (
                                                msg
                                    ...
                                    context = {
                                        ...
                                        "message_object": msgObj,
                                        "thread_id": str(
                                            int(msgObj.idTo) or msgObj.idTo
                                        ),
                                        "thread_type": ThreadType.GROUP,
                                    }
                                    context = ContextObject.fromDict(context)
                                    ...
                                    if control["content"]["act_type"] == "group":
                                        ...
                                        [
                                            (
                                                ...
                                                if thread
                                                else await self.on_event(context)
                                            )
                                        ]
    ...
    async def on_message(self, ctx):
        ...
        """
        logger.info(
            "{} from {} in {}".format(ctx.message, ctx.thread_id, ctx.thread_type.name)
        )
