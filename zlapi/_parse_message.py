...
def parse_markdown(text):
    ...
    markdown_elements = []
    markdown = {"**": "bold", "__": "underline", "_": "italic", "~~": "strike"}

    temp_text = text
    while any(temp_text.count(char) >= 2 for char in markdown.keys()):

        markup_start_positions = {
            char: temp_text.find(char) for char in markdown.keys()
        }

        sorted_markup_start_positions = dict(
            sorted(markup_start_positions.items(), key=lambda item: item[1])
        )
        ...
        for char, start in sorted_markup_start_positions.items():
            ...
            temp_text = (
                temp_text[:start]
                + temp_text[start + len(char) : end]
                + temp_text[end + len(char) :]
            )
            break
    ...
    for element in markdown_elements:
        text = (
            text[: element["start"]]
            + text[element["start"] + element["char_len"] : element["end"]]
            + text[element["end"] + element["char_len"] :]
        )
        element["start"] -= 1
        element["end"] += 2
...
def parse_html(text):
    ...
    markdown_elements = []
    markdown = {"<b>": "bold", "<u>": "underline", "<i>": "italic", "<s>": "strike"}

    temp_text = text
    while any(
        temp_text.count(char) >= 1 and temp_text.count("</" + char[1:]) >= 1
    ...
    for element in markdown_elements:
        text = (
            text[: element["start"]]
            + text[element["start"] + element["char_len"] : element["end"]]
            + text[element["end"] + element["char_len"] + 1 :]
        )
        element["start"] -= 1
