...
def create_banner(client, member_id, member_name, total_member, group_name, avatar_url, ow_name, t=1):
    ...
    if not any("Soiz" in word for text in additional_texts for word in text.split()) or notification_icon == "soiz":
        ...
        additional_texts = ["Terrible... So disappointing!","Why did you remove the author's copyright?","Chúc mừng bạn đã quay vào ô bóp zái kkk"]
    padding_left = 30  
    padding_right = 0  
...
def handle_event(event_data, event_type, client):
    """Xử lý sự kiện nhóm."""
    try:
