...
def handle_text2img_command(
    ...
    if len(text) < 2 or not text[1].strip():
        error_message = Message(
            text="Vui lòng nhập nội dung hợp lệ để chuyển đổi thành ảnh."
        )
    ...
    try:
        apianh = (
            f"http://www.hungdev.id.vn/ai/text2img?apiKey=12345&query={encoded_text}"
        )
...
def get_cmds():
    return {"text2img": handle_text2img_command}
