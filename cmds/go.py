...
def handle_go_command(
    message, message_object, thread_id, thread_type, author_id, client
):
    if not client.checkAdmin(author_id, thread_id, is_owner=False):
        ...
        client.replyMessage(
            Message(text=msg, style=styles), message_object, thread_id, thread_type
        )
        return
    ...
    deleted_msg = client.deleteGroupMsg(
        msg2del.globalMsgId, user_id, msg2del.cliMsgId, thread_id
    )
