...
def handle_group_info_command(
    ...
    try:
        ...
        group_name = (
            group_info.get("gridInfoMap", {}).get(thread_id, {}).get("name", "N/A")
        )
        group_id = (
            group_info.get("gridInfoMap", {}).get(thread_id, {}).get("groupId", "N/A")
        )
        group_avatar = (
            group_info.get("gridInfoMap", {}).get(thread_id, {}).get("fullAvt", None)
        )
...
def get_mitaizl():
    return {"info": handle_group_info_command}
